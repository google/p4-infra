...
// TODO: b/499072223 - Update function to use scope rather than annotation.
...
StatusOr<std::string> GetPacketIoMessage(const IrP4Info& info) {
  ...
  for (const auto& [name, meta] :
       AsOrderedView(info.packet_in_metadata_by_name())) {
  ...
  absl::StrAppend(&result, "  message Metadata {\n");
  for (const auto& [name, meta] :
       AsOrderedView(info.packet_out_metadata_by_name())) {
    // Skip PI-only padding.
    if (meta.is_padding()) continue;
    ASSIGN_OR_RETURN(
        const std::string meta_name,
